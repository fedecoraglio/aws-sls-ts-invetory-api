...
// src/functions/roles/list/handler.ts
var handler_exports = {};
...
// src/libs/middleware.ts
var MiddlewareFunction = import_core.default.MiddlewareFn;
var apiGatewayResponseMiddleware = (options = {}) => {
  const after = async (request) => {
    var _a;
    if (!((_a = request.event) == null ? void 0 : _a.httpMethod) || request.response === void 0 || request.response === null) {
      return;
    }
...
// src/libs/lambda.ts
var middyfy = (handler) => {
  return (0, import_core2.default)(handler).use((0, import_http_json_body_parser.default)()).use(apiGatewayResponseMiddleware({
    enableErrorLogger: process.env.IS_OFFLINE === "true"
  }));
};

// src/dbconnect/index.ts
...
var dbconnect_default = dynamoDBClient;
...
var RoleRepository = class {
  ...
  static getInstance() {
    if (!RoleRepository.instance) {
      RoleRepository.instance = new RoleRepository(dbconnect_default());
    }
    return RoleRepository.instance;
  }
  async getRoles() {
    try {
      const resp = await this.docClient.scan({
        TableName: tableName
      }).promise();
      return {
        count: resp.Count,
...
var RoleBuilder = class {
  ...
  transformModelsToDtos(models) {
    return models.map((dto) => this.transformModelToDto(dto));
  }
  transformDtosToModels(dtos) {
...
// src/functions/roles/list/handler.ts
var listRole = async (_event, _context) => {
  console.log(`Getting roles`);
  return await new RoleService().getRoles();
};
var main = middyfy(listRole);
